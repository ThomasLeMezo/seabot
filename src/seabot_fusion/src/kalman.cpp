...
Matrix<double,NB_STATES, 1> f(const Matrix<double,NB_STATES,1> &x, const Matrix<double,NB_STATES, 1> &u){
  ...
  y(0) = -coeff_A*(x(2)+x(3)-coeff_compressibility*x(1))-coeff_B*copysign(pow(x(0), 2), x(0));
  y(1) = y(0);
  y(2) = u(2);
  y(3) = 0.0;
  return y;
...
void kalman_predict(const Matrix<double,NB_STATES, 1> &xup,
                    ...
                    Matrix<double,NB_STATES, NB_STATES> &gamma_1){
  gamma_1 = Ak*Gup*Ak.transpose()+gamma_alpha;
  xnew = xup + f(xup, u)*dt;
}
...
void kalman(const Matrix<double,NB_STATES, 1> &x0,
            const Matrix<double,NB_STATES,NB_STATES> &gamma_0,
            const Matrix<double,NB_STATES, 1> &u,
            const Matrix<double,NB_MESURES, 1> &y,
            const Matrix<double,NB_STATES, NB_STATES> &gamma_alpha,
            ...
            const Matrix<double,NB_MESURES, NB_STATES> &C,
            const double &dt,
            Matrix<double,NB_STATES, 1> &x1,
            Matrix<double,NB_STATES,NB_STATES> gamma_1
            ){
  Matrix<double,NB_STATES, 1> xup;
  Matrix<double,NB_STATES, NB_STATES> Gup;
  kalman_correc(x0,gamma_0,y,gamma_beta,C, xup,Gup);
  kalman_predict(xup,Gup,u,gamma_alpha,Ak, dt, x1, gamma_1);
}
...
int main(int argc, char *argv[]){
  ...
  gamma_alpha(2,2) = pow(1e-3, 2); // Piston Volume (m3)
  gamma_alpha(3,3) = pow(1e-5, 2); // Error offset;

  gamma_beta(0, 0) = pow(1e-3, 2); // Depth
  gamma_beta(1, 1) = pow(1e-5, 2); // Piston Volume
  ...
  while (ros::ok()){
    ...
    if(depth>0.5){
      ...
      kalman(xhat,gamma,command,measure,gamma_alpha,gamma_beta,Ak,Ck, dt, xhat, gamma_1);
    }
    else{
      xhat(0) = 0.0;
      xhat(1) = depth;
    ...
    msg.covariance[0] = gamma_1(0,0);
    msg.covariance[1] = gamma_1(1,0);
    msg.covariance[2] = gamma_1(2,0);
    msg.covariance[3] = gamma_1(3,0);
    msg.covariance[4] = gamma_1(0,1);
    msg.covariance[5] = gamma_1(1,1);
    msg.covariance[6] = gamma_1(2,1);
    msg.covariance[7] = gamma_1(3,1);
    msg.covariance[8] = gamma_1(0,2);
    msg.covariance[9] = gamma_1(1,2);
    msg.covariance[10] = gamma_1(2,2);
    msg.covariance[11] = gamma_1(3,2);
    msg.covariance[12] = gamma_1(0,3);
    msg.covariance[13] = gamma_1(1,3);
    msg.covariance[14] = gamma_1(2,3);
    msg.covariance[15] = gamma_1(3,3);
