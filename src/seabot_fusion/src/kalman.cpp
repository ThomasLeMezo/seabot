...
int main(int argc, char *argv[]){
  ...
  const double m = n.param<double>("/m", 9.045); //modification a faire
  ...
  const double piston_diameter = n.param<double>("/piston_diameter", 0.05); //modification a faire  0.0195
  const double piston_ref_eq = n.param<double>("/piston_ref_eq", 2100); //modification a faire

  const double estimated_first_error_equilibrium_tick = n_private.param<double>("estimated_first_error_equilibrium_tick", 250); //modification a faire
  ...
  while (ros::ok()){
    ...
    if(depth>limit_min_depth && (t-time_last_depth).toSec()<0.1 && depth_valid){
      ...
      measure(0) = depth;
      command(0) = (piston_ref_eq - piston_position)*tick_to_volume; //modification a faire
