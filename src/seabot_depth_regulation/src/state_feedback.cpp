...
int main(int argc, char *argv[]){
  ...
  const double diam_collerette = n.param<double>("/diam_collerette", 0.24); //modification a faire
  const double screw_thread = n.param<double>("/screw_thread", 1.75e-3); //modification a faire
  const double tick_per_turn = n.param<double>("/tick_per_turn", 48); //modification a faire
  const double piston_diameter = n.param<double>("/piston_diameter", 0.05); //modification a faire
  const double piston_ref_eq = n.param<double>("/piston_ref_eq", 2100); //modification a faire
  const double piston_max_value = n.param<double>("/piston_max_value", 2400); //modification a faire
  const double Cf = M_PI*pow(diam_collerette/2.0, 2); //modification a faire
  tick_to_volume = (screw_thread/tick_per_turn)*pow(piston_diameter/2.0, 2)*M_PI; //modification a faire
  coeff_A = g*rho/m;
  ...
  const double piston_max_velocity = n.param<double>("piston_speed_max_tick", 30)*tick_to_volume; // in m3/sec //modification a faire

  /// ************************* ROS Communication *************************
  // Subscriber
  ros::Subscriber kalman_sub = n.subscribe("/fusion/kalman", 1, kalman_callback); //modification a faire
  ros::Subscriber state_sub = n.subscribe("/driver/piston/state", 1, piston_callback); //modification a faire
  ros::Subscriber depth_set_point_sub = n.subscribe("/mission/set_point", 1, depth_set_point_callback);

  // Publisher
  ros::Publisher position_pub = n.advertise<seabot_piston_driver::PistonPosition>("/driver/piston/position", 1); //modification a faire
    ...
    if(depth_set_point>=limit_depth_regulation && !emergency){
      ...
      case STATE_SURFACE:
        ...
        if(x(1)<limit_depth_regulation)
          u = -speed_volume_sink*tick_to_volume; //modification a faire
        else
          regulation_state = STATE_REGULATION;

        if(piston_position >= piston_set_point*0.99)
          piston_set_point = piston_set_point - u/(tick_to_volume*frequency); //modification a faire
        else
          piston_set_point = max(piston_ref_eq, piston_position); //modification a faire
        break;
      ...
      case STATE_REGULATION:
        if(x(1)>=limit_depth_regulation){
          if((ros::Time::now()-time_last_state).toSec()<1.0){
            ...
            x(2) = (piston_ref_eq - piston_position)*tick_to_volume; //modification a faire
            ...
            if((piston_switch_in && u<0) || (piston_switch_out && u>0)) //modification a faire
              u = 0.0;
          }
          else{
        ...
        if(abs(u)>piston_max_velocity){ //modification a faire
          u=copysign(piston_max_velocity, u); //modification a faire
        }

        piston_set_point = piston_position - u/(tick_to_volume*frequency); //modification a faire
        break;
      default:
        break;
      }
      ...
      if(abs(piston_position_old - piston_set_point)>hysteresis_piston){ //modification a faire
        if(piston_set_point>piston_max_value) //modification a faire
          piston_set_point = piston_max_value; //modification a faire
        if(piston_set_point<0) //modification a faire
          piston_set_point = 0; //modification a faire

        position_msg.position = round(piston_set_point); //modification a faire
        piston_position_old = piston_set_point; //modification a faire
      }

      // Debug msg
      debug_msg.u = u;
      debug_msg.piston_set_point = piston_set_point; //modification a faire
