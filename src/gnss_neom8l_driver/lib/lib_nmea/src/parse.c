...
int nmea_parse_PRTI01(const char *s, const int len, bool has_checksum, nmeaPRTI01 *pack) {
   ...
   * Clear before parsing, to be able to detect absent fields
   */
  int invalid_field = -99999;
  pack->present   = 0;     /**< Mask specifying which fields are present, same as in nmeaINFO */
  pack->utc       = invalid_field;          /**< UTC of sample */
  pack->sample_nb = invalid_field;        /**< Sample number */
  pack->temp      = invalid_field;         /**< Temperature in hundreds of degrees Celsius. Measured at the transducer*/
  pack->bottom_vx = invalid_field;      /**< Bottom track X velocity component mm/s. -99999 indicates no valid velocity*/
  pack->bottom_vy = invalid_field;      /**< Bottom track Y velocity component mm/s. -99999 indicates no valid velocity*/
  pack->bottom_vz = invalid_field;      /**< Bottom track Z velocity component mm/s. -99999 indicates no valid velocity*/
  pack->bottom_depth    = invalid_field;     /**< Depth below transducer in mm. Range to the bottom, 0 = no detection*/
  pack->water_vx  = invalid_field;       /**< Water mass X velocity component mm/s. -99999 indicates no valid velocity*/
  pack->water_vy  = invalid_field;       /**< Water mass Y velocity component mm/s. -99999 indicates no valid velocity*/
  pack->water_vz  = invalid_field;       /**< Water mass Z velocity component mm/s. -99999 indicates no valid velocity*/
  pack->water_depth   = invalid_field;      /**< Depth of water mass measurement in mm. Position of bin in front of the transducer*/
  ...
  if (!(pack->bottom_vx==invalid_field) && !(pack->bottom_vy==invalid_field) && !(pack->bottom_vz==invalid_field)) {
