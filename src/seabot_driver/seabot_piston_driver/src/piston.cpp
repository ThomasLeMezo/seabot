...
//addition
void Piston::set_piston_speed_in(const __u8 &speed_in) const{
...
//addition
void Piston::set_piston_speed_out(const __u8 &speed_out) const{
...
void Piston::get_piston_all_data(){
  ...
  if(m_position >= 1390.0){ //max : 1410.5
	m_switch_in = true;
  }
  else{
	m_switch_in = false;
  ...
  //m_switch_out = false; //buff[2] & 0b1; //non utilise
  //m_switch_in = false; //(buff[2] >> 1) & 0b1; //non utilise
  m_state = (buff[2] >> 2) & 0b11;
  m_motor_on = true; //(buff[2] >> 4) & 0b1; //non utilise
  //ROS_INFO("m_motor_on value : %d", m_motor_on);
  m_enable_on = (buff[2] >> 5) & 0b1; 
  m_position_set_point = (buff[4] << 8 | buff[3])/4.0;
  m_motor_speed = buff[5];


  m_switch_halfway = !i2c_smbus_read_byte_data(m_file, 0xB1);
